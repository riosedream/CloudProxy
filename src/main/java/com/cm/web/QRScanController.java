...
public class QRScanController {
    ...
    public String index(Model model) throws UnknownHostException {
        ...
        // 获取作为Server的本机IP
        String localServerIP = HostUtil.getLocalServer().getHostAddress();
     ...
     */
    @RequestMapping("/")
    public String index2(Model model) throws UnknownHostException {
        String uuid = UUID.randomUUID().toString();
        // 获取Server的本机IP
        String localServerIP = HostUtil.getLocalServer().getHostAddress();
        PoolCache.cacheMap.put(uuid, new ScanPool());
    ...
    @ResponseBody
    public JSONObject scanLogin(String uuid, String phoneId){
        ...
        if (pool == null) {
            ...
            pool.setScanSuccess();
            obj.put("msg","扫码成功!"); // 返回给扫码手机的浏览器页面
    ...
    public String success(){
        return "扫码成功!!"; // 响应给二维码Web端页面
    }

    /**
     * 给Web页面 index.jsp 定时器，查询扫码状态请求
     * 查询扫码状态
     * @return
     */
    @RequestMapping("/pool")
    ...
    public JSONObject pool(String uuid){
        ...
        try {
            if (pool == null) {
                ...
                obj.put("successFlag","0"); // 返给index.jsp 页面中keepPool()中 msg.successFlag == '0'情况
                obj.put("msg","该二维码已经失效,请重新获取");
            } else {
